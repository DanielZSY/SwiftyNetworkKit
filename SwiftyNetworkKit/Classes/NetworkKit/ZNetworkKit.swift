...
public struct ZNetworkKit {
    ...
    public static var created: ZNetworkKit {
        return ZNetworkKit()
    }
    /// 网络请求对象
    ...
    /// - returns: 返回值描述
    @discardableResult
    public func startRequest(target: ZNetworkTargetType, responseBlock: @escaping ((_ result: ZNetworkResult) -> (Void))) -> Cancellable {
        ...
        let task = provider.request(target, progress: nil, completion: { (result) in
            ...
            case .success(let response):
                do {
                    ...
                    case ZEnumNetworkCode.UnAuthorized.rawValue: NotificationCenter.default.post(name: NSNotification.Name.init(rawValue: "ApiUnAuthorized"), object: nil)
                    case ZEnumNetworkCode.Balance.rawValue: NotificationCenter.default.post(name: NSNotification.Name.init(rawValue: "InsufficientBalance"), object: nil)
                    case ZEnumNetworkCode.LoginExpired.rawValue: NotificationCenter.default.post(name: NSNotification.Name.init(rawValue: "LoginExpired"), object: nil)
                    default: break
                    }
